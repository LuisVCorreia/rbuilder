...
impl UsedStateTrace {
    ...
    pub fn price_displacement(&self) -> HashMap<Address, f64> {
        ...
        let mask160 = (U256::from(1u64) << 160) - U256::from(1u64);
            ...
            let displacement = match kind {
                PoolKind::UniV3 => {
                    ...
                    let before = self.read_slot_values.get(&key)
                        .map(|v| u256_to_f64(U256::from_be_bytes(v.0) & mask160));
                    let after = self.written_slot_values.get(&key)
                        .map(|v| u256_to_f64(U256::from_be_bytes(v.0) & mask160));
                    match (before, after) {
                        // TODO: square the price? For large price changes this might matters
