    ...
    OrdersSourceType: OrdersSource<ConfigType, ProviderType>,
{
    ...
    // build the JSON output for the groups
    let block_number = ctx.evm_env.block_env.number;

    // helper to serialize an order's nonces
    fn serialize_nonces(o: &Order) -> Vec<serde_json::Value> {
        o.nonces()
            .into_iter()
            .map(|n| json!({
    ...
    // Build groups array: stable sort of order_ids inside each group, include nonces and distinct_senders
    let mut groups_json: Vec<serde_json::Value> = Vec::with_capacity(groups.len());
    for (gid, g) in groups.iter().enumerate() {
        // Extract (order_id, &Order) pairs from the group
        let mut entries: Vec<(OrderId, &Order)> =
            g.orders.iter().map(|so| (so.order.id(), &so.order)).collect();

        // Stable sort by order_id for determinism
        entries.sort_by_key(|(oid, _)| *oid);

        // Build order_ids and full order entries (with nonces)
        ...
        for (oid, ord) in entries {
            ...
            // collect nonces + track distinct senders
            for n in ord.nonces() {
                distinct_addrs.insert(n.address);
            }
    ...
    // Write file
    let outdir = PathBuf::from("conflict_testing/parallel_builder_call_end");
    fs::create_dir_all(&outdir)?;
    let path = outdir.join(format!("parallel_conflicts_{}.json", block_number));
    fs::write(&path, serde_json::to_string_pretty(&root)?)?;
    println!("Saved parallel groups to {}", path.display());
