...
/// The factory now holds a shared provider and the cache database.
#[derive(Clone)]
...
impl HttpStateProviderFactory {
    /// Creates a new factory with a specified cache path.
    pub fn new_with_url_and_cache(url: &str, cache_path: PathBuf) -> Self {
        let provider = ProviderBuilder::new()
            .network::<Ethereum>()
            .on_http(url.parse().expect("Failed to parse provider URL"));

        // Since StateCache::new is now async, we must block on it here.
        let state_cache = tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(
                StateCache::new(&cache_path)
            )
...
impl StateProviderFactory for HttpStateProviderFactory {
    ...
    fn history_by_block_hash(&self, block_hash: B256) -> ProviderResult<StateProviderBox> {
        ...
        let res = tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                provider.get_block_by_hash(block_hash).await
            })
        })
        .map_err(|e| ProviderError::Other(AnyError::new(e)))?
        .ok_or_else(|| ProviderError::Other(AnyError::new(std::io::Error::new(
...
/// The state provider now holds the cache database.
...
impl HttpStateProvider {
    /// Updated `new` to accept the cache database.
    pub fn new(
        provider: Arc<dyn Provider<Ethereum> + Send + Sync>,
        hash: B256,
        cache_db: CacheDB,
    ...
    fn storage(&self, address: Address, storage_key: StorageKey) -> ProviderResult<Option<StorageValue>> {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                // 2. If miss, fetch from RPC
                let block_id = BlockId::hash(self.hash);
                let res = self.provider.get_storage_at(address, storage_key.into()).block_id(block_id).await
                    .map_err(|e| ProviderError::Other(AnyError::new(e)))?;
    ...
    fn bytecode_by_hash(&self, code_hash: &B256) -> ProviderResult<Option<Bytecode>> {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                let code: Option<Bytes> = self.provider.client().request("debug_codeByHash", (code_hash_val, block_id)).await
                    .map_err(|e| ProviderError::Other(AnyError::new(e)))?;
    ...
    fn block_hash(&self, number: BlockNumber) -> ProviderResult<Option<B256>> {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                // 2. If miss, fetch from RPC
                let block = self.provider.get_block_by_number(BlockNumberOrTag::Number(number)).await
                    .map_err(|e| ProviderError::Other(AnyError::new(e)))?;
    ...
    fn basic_account(&self, address: &Address) -> ProviderResult<Option<Account>> {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                // 2. If miss, fetch from RPC concurrently
                let block_id = BlockId::hash(self.hash);
...
// The following trait implementations do not need changes.
