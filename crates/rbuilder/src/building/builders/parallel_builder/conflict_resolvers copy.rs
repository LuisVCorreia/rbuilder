...
/// Return nonce-aware chains of order indices (one per sender),
/// but deduplicate by (sender, nonce) choosing the best candidate.
/// "Best" = max coinbase_profit, then max mev_gas_price, then lowest index.
...
fn allowed_indices_after_nonce_dedup(group: &super::ConflictGroup) -> Option<HashSet<usize>> {
    let chains = build_sender_chains(group)?;
...
/// Enumerate all interleavings of k chains (each inner vec is a fixed order).
/// Use backtracking; safe for small counts (we limit this to small multinomials).
fn enumerate_all_interleavings(chains: &[Vec<usize>], cap: usize) -> Vec<Vec<usize>> {
    let k = chains.len();
    if k == 0 {
        return vec![];
    }
    ...
    fn dfs(
        ...
        // Branch on any chain with remaining elements
...
fn sample_one_uniform_interleaving(chains: &[Vec<usize>], rng: &mut rand::rngs::SmallRng) -> Vec<usize> {
    ...
    for _ in 0..total {
        ...
        // Integer "roulette wheel" over remaining lengths => exact proportional probabilities
        ...
        while let Some(order_idx) = remaining_orders.pop() {
            ...
            match partial_block.commit_order(
                ...
                Ok(res) => {
                    ...
                    // let prefix_ids: Vec<OrderId> = sequenced_order_result
                    //     .iter()
                    //     .map(|(idx, _)| task.group.orders[*idx].order.id())
                    //     .collect();

                    // let (bundle_state, _) = state.clone().into_parts();

                    // self.simulation_cache.try_store_if_absent(
                    //     &prefix_ids,
...
fn generate_random_permutations(task: &ConflictTask, seed: u64, count: usize) -> Vec<Vec<usize>> {
    ...
    if let Some(chains) = build_sender_chains(order_group) {
...
fn generate_all_permutations(task: &ConflictTask) -> Vec<Vec<usize>> {
    ...
    // // Nonce-aware: enumerate all interleavings of chains
    // if let Some(chains) = build_sender_chains(order_group) {
    //     return enumerate_all_interleavings(&chains, ALL_PERMS_CAP);
    // }

    // // Fallback: naive permutations for legacy/bundles
