use crate::{
    ...
    primitives::{OrderId, SimulatedOrder, Order},
};
...
pub enum ConflictCause {
    /// Read/write overlap on *nonce* (EOA, slot 0) – we detect the slot-0-on-signer case
    Nonce,
    /// Read/write overlap on any storage slot (non-nonce)
    StorageRW,
    /// Balance writer (send/receive) overlaps balance reader on the same address
    BalanceRW,
    /// Contract create/destroy overlaps any storage access on that address
    CodeVsStorage,
    /// Both try to create/destroy the same address
    CodeVsCode,
}

#[derive(Debug, Clone, Serialize)]
pub struct PairConflictRecord {
    pub u1: String,
    pub u2: String,
...
pub struct ConflictFinder {
    ...
    order_signers: HashMap<OrderId, Vec<Address>>,  // tx signers (for nonce detection)

    idx_storage_reads: HashMap<SlotKey, Vec<OrderId>>,
    idx_storage_writes: HashMap<SlotKey, Vec<OrderId>>,
    idx_storage_reads_by_addr: HashMap<Address, Vec<OrderId>>,
    idx_storage_writes_by_addr: HashMap<Address, Vec<OrderId>>,

    idx_balance_reads: HashMap<Address, Vec<OrderId>>,
    idx_balance_writes: HashMap<Address, Vec<OrderId>>, // sent/received

    idx_code_writes: HashMap<Address, Vec<OrderId>>,  // created+destructed

    pair_conflicts: HashMap<(OrderId, OrderId), HashSet<ConflictCause>>,
}
...
impl ConflictFinder {
    pub fn new() -> Self {
        ConflictFinder {
            ...
            order_signers: HashMap::default(),
            idx_storage_reads: HashMap::default(),
            idx_storage_writes: HashMap::default(),
            idx_storage_reads_by_addr: HashMap::default(),
            idx_storage_writes_by_addr: HashMap::default(),
            idx_balance_reads: HashMap::default(),
            idx_balance_writes: HashMap::default(),
            idx_code_writes: HashMap::default(),
            pair_conflicts: HashMap::default(),
        }
    }

    #[inline]
    fn extract_senders(&self, o: &Order) -> Vec<Address> {
        o.nonces().into_iter().map(|n| n.address).collect()
    }

    #[inline]
    fn is_nonce_slot_for_either(
        &self,
        addr: Address,
        key: &B256,
        cur_senders: &[Address],
        ...
        // slot 0 and address is signer of current or other
        if *key != B256::default() {
            return false;
        }
        if cur_senders.iter().any(|&a| a == addr) {
            return true;
        }
        if let Some(others) = self.order_signers.get(other_id) {
            return others.iter().any(|&a| a == addr);
        }
        false
    }

    #[inline]
    fn record_pair_conflict(
        &mut self,
        a: &OrderId,
        b: &OrderId,
        cause: ConflictCause,
    ...
    pub fn pair_conflicts_map(
        &self
    ) -> &HashMap<(OrderId, OrderId), HashSet<ConflictCause>> {
        &self.pair_conflicts
    }

    /// Convenience: make a serializable flat vector for writing to JSON.
    pub fn pair_conflicts_records(&self) -> Vec<PairConflictRecord> {
        let mut out = Vec::with_capacity(self.pair_conflicts.len());
        for ((u1, u2), causes) in &self.pair_conflicts {
            out.push(PairConflictRecord {
                u1: u1.to_string(),
                u2: u2.to_string(),
                causes: causes.iter().copied().collect(),
            });
        }
        out
    }
    ...
    pub fn add_orders(&mut self, orders: Vec<Arc<SimulatedOrder>>) {
        for order in orders {
            ...
            let oid = order.id();
            let cur_senders = self.extract_senders(&order.order);
            self.order_signers.insert(oid.clone(), cur_senders.clone());

            for (slot_key, _) in &used_state.read_slot_values {
                if let Some(others) = self.idx_storage_writes.get(slot_key) {
                    let others: Vec<OrderId> = others.clone();
                    for other in others {
                        let cause = if self.is_nonce_slot_for_either(
                            slot_key.address,
                            &slot_key.key,
                            &cur_senders,
                            &other,
                            ...
                            ConflictCause::StorageRW
                        };
                        self.record_pair_conflict(&oid, &other, cause);
                    }
                }
                // Code vs Storage
                if let Some(others) = self.idx_code_writes.get(&slot_key.address) {
                    let others: Vec<OrderId> = others.clone();
                    for other in others {
                        self.record_pair_conflict(&oid, &other, ConflictCause::CodeVsStorage);
                    }
                }
            }

            for (slot_key, _) in &used_state.written_slot_values {
                if let Some(others) = self.idx_storage_reads.get(slot_key) {
                    let others: Vec<OrderId> = others.clone();
                    for other in others {
                        let cause = if self.is_nonce_slot_for_either(
                            slot_key.address,
            ...
            {
                let mut bw_addrs: Vec<Address> = used_state
                    .sent_amount
                    .keys()
                    .chain(used_state.received_amount.keys())
                    .cloned()
                    .collect();
                bw_addrs.sort_unstable();
                bw_addrs.dedup();

                for addr in bw_addrs {
                    if let Some(others) = self.idx_balance_reads.get(&addr) {
                        let others: Vec<OrderId> = others.clone();
                        for other in others {
                            self.record_pair_conflict(&oid, &other, ConflictCause::BalanceRW);
                        }
                    }
                }
            }

            // Balance: current READ vs prior WRITE
            for addr in used_state.read_balances.keys() {
                if let Some(others) = self.idx_balance_writes.get(addr) {
                    let others: Vec<OrderId> = others.clone();
                    for other in others {
                        self.record_pair_conflict(&oid, &other, ConflictCause::BalanceRW);
                    }
            ...
            // Code writes: current CREATE/DESTROY vs prior storage access & code writes
            {
                let mut code_addrs: Vec<Address> = used_state
                    .created_contracts
                    .iter()
                    .chain(used_state.destructed_contracts.iter())
                    .cloned()
                    .collect();
                ...
                for addr in code_addrs {
                    // vs prior storage (reads/writes) at same address
                    if let Some(others) = self.idx_storage_reads_by_addr.get(&addr) {
                        let others: Vec<OrderId> = others.clone();
                        for other in others {
                            self.record_pair_conflict(&oid, &other, ConflictCause::CodeVsStorage);
                        }
                    }
                    if let Some(others) = self.idx_storage_writes_by_addr.get(&addr) {
                        let others: Vec<OrderId> = others.clone();
                        for other in others {
                            self.record_pair_conflict(&oid, &other, ConflictCause::CodeVsStorage);
                        }
                    }
                    // vs prior code writes at same address
                    if let Some(others) = self.idx_code_writes.get(&addr) {
                        let others: Vec<OrderId> = others.clone();
                        for other in others {
                            self.record_pair_conflict(&oid, &other, ConflictCause::CodeVsCode);
                        }
                    }
                }
            }
            ...
            // storage reads
            for slot_key in used_state.read_slot_values.keys() {
                self.idx_storage_reads.entry(slot_key.clone()).or_default().push(oid.clone());
                self.idx_storage_reads_by_addr.entry(slot_key.address).or_default().push(oid.clone());
            }
            // storage writes
            for slot_key in used_state.written_slot_values.keys() {
                self.idx_storage_writes.entry(slot_key.clone()).or_default().push(oid.clone());
                self.idx_storage_writes_by_addr.entry(slot_key.address).or_default().push(oid.clone());
            }
            // balance reads
            for addr in used_state.read_balances.keys() {
                self.idx_balance_reads.entry(*addr).or_default().push(oid.clone());
            }
            // balance writes (send/receive)
            {
                let mut bw_addrs: Vec<Address> = used_state
                    .sent_amount
                    .keys()
                ...
                bw_addrs.dedup();
                for addr in bw_addrs {
                    self.idx_balance_writes.entry(addr).or_default().push(oid.clone());
                }
            }
            // code writes
            {
                ...
                code_addrs.dedup();
                for addr in code_addrs {
                    self.idx_code_writes.entry(addr).or_default().push(oid.clone());
                }
            }
...
mod tests {
    ...
    #[test]
    fn pairwise_storage_rw_is_recorded() {

        let mut gen = DataGenerator::new();
        let slot = gen.create_slot();
        let oa = gen.create_order(Some(&slot), None, None, None, None, None); // reader
        let ob = gen.create_order(None, Some(&slot), None, None, None, None); // writer
        let mut f = ConflictFinder::new();
        f.add_orders(vec![oa.clone(), ob.clone()]);

        let pairs = f.pair_conflicts_map();
        assert_eq!(pairs.len(), 1);
        let (id_a, id_b) = if oa.order.id() <= ob.order.id() {
            (oa.order.id(), ob.order.id())
        } else {
            (ob.order.id(), oa.order.id())
        };
        let causes = pairs.get(&(id_a, id_b)).unwrap();
        assert!(causes.contains(&ConflictCause::StorageRW));
    }

    #[test]
    fn pairwise_code_vs_code_is_recorded() {
        let mut gen = DataGenerator::new();
        let addr = gen.create_slot().address;
        let oa = gen.create_order(None, None, None, None, None, Some(&addr));
        let ob = gen.create_order(None, None, None, None, None, Some(&addr));
        let mut f = ConflictFinder::new();
        f.add_orders(vec![oa.clone(), ob.clone()]);
        ...
        let causes = pairs.get(&(id_a, id_b)).unwrap();
        assert!(causes.contains(&ConflictCause::CodeVsCode));
    }
