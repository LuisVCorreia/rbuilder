...
struct OrderDesc {
    id: OrderId,
    profit: U256,       // keep as U256
    blob_gas: u64,
    signer: Option<Address>,
    nonce: Option<u64>,
}

pub fn build_descriptors(sim_orders: &[Arc<SimulatedOrder>]) -> Vec<OrderDesc> {
...
/// Choose the “best” normal at a nonce (highest profit).
fn choose_best_normal<'a>(normals: &[&'a OrderDesc]) -> &'a OrderDesc {
    let mut best = normals[0];
    for &d in normals.iter().skip(1) {
        if d.profit > best.profit {
            best = d;
        }
    }
    best
...
struct RepNode<'a> {
    nonce: u64,
    rep:   &'a OrderDesc, // chosen_blob or best normal
    is_blob: bool,
}

/// Value/gas step contributed by selecting a blob at position `bi` (inclusive) up to next blob (exclusive).
#[derive(Clone, Debug)]
struct Step {
    delta_v: U256,   // sum of representative profits in that segment
    delta_w: u64,    // blob gas of the starting blob
    cutoff_nonce: u64, // nonce of this blob (used as “first discarded” if not selected)
}
...
fn prepare_classes<'a>(
    ...
    for (signer, mut chain_raw) in by_signer {
        ...
        for (j, &bi) in blob_idx.iter().enumerate() {
            ...
            steps.push(Step {
                ...
                cutoff_nonce: rep_chain[bi].nonce,
            });
        }

        classes.push(Class { signer, raw_groups, rep_chain, blob_idx, steps });
    }
...
/// Returns (chosen_prefix_per_class, keep_set BUILT FROM RAW GROUPS with the rules described above).
